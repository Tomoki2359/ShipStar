...
float4 PS(VS_OUT inData) : SV_Target
{
	...
	if (isTexture == true)
	{
		...
		ambient = g_texture.Sample(g_sampler, inData.uv) * float4(brightness, brightness, brightness, 1.0);
	}
	else
	{
		diffuse = float4(red, green, blue, alpha) * diffuseColor * inData.color;
		ambient = diffuseColor * float4(brightness, brightness, brightness, 1.0);
	}
